...
//
//	-- Fit  with gamma distribution of the new histo with F(N_s, N_b, x) = N_s*pfd_s(x)+N_b*pfd_b(x)
...
double BplusS(double * x, double * p ){
	return p[0]*gammadist(x, &p[1]) + p[3]*gammadist(x, &p[4]);
}
...
//	min=0; max=500; nbin=500;	
	...
	TF1 * pdf_funzs = new TF1("pdf_funzs", gammadist, min, max, 2);
	TF1 * pdf_funzb = new TF1("pdf_funzb", gammadist, min, max, 2);
	pdf_funzs->SetParameter(0, 2.);
	pdf_funzs->SetParameter(1, 20.);
	pdf_funzb->SetParameter(0, 2.);
	pdf_funzb->SetParameter(1, 20.);
	pdf_funzs->SetParName(0, "k_s");
	pdf_funzs->SetParName(1, "Theta_s");
	pdf_funzb->SetParName(0, "k_b");
	pdf_funzb->SetParName(1, "Theta_b");
	hs->Fit("pdf_funzs", "RQ");
	hb->Fit("pdf_funzb", "RQ");
	setstack( hs, hb, dist);
...
//	double lum [4] = { 375., 750., 1500., 3000. };	
	...
	for(int i=0; i< 4; i++){
		...
		TF1 * funz_bs = new TF1("funz_bs", BplusS, min, max, 6);
		funz_bs->SetParameter(0, evs_exp);
		funz_bs->SetParameter(3, evb_exp);
		funz_bs->SetParameter(1, pdf_funzs->GetParameter(0));
		funz_bs->SetParameter(4, pdf_funzb->GetParameter(0));
		funz_bs->SetParameter(2, pdf_funzs->GetParameter(1));
		funz_bs->SetParameter(5, pdf_funzb->GetParameter(1));
		funz_bs->SetParName(0, "N_s");
		funz_bs->SetParName(1, "k_s");
		funz_bs->SetParName(2, "Theta_s");
		funz_bs->SetParName(3, "N_b");
		funz_bs->SetParName(4, "k_b");
		funz_bs->SetParName(5, "Theta_b");
