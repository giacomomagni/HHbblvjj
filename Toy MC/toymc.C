...
//  double lum [4] = { 100., 150., 300., 3000. };   
    ...
    for(int i=0; i<4; i++){
        ...
        for(int k=1; k <= Nsim; k++ ){
            ...
            for (int j=2; j<dof+3; j++)      funz_bs->FixParameter(j, pdf_funzs->GetParameter(j-2));
            for (int j=dof+3; j<2*dof+4; j++)    funz_bs->FixParameter(j, pdf_funzb->GetParameter(j-(dof+3)));
             
            bs->Fit("funz_bs", "QRLMN");
            //cout<< funz_bs->GetParameter(0) <<endl;
            histo_Ns->Fill(funz_bs->GetParameter(0));         
        }
