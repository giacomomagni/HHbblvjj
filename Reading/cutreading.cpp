//read the file HH.lhe and ttbar.lhe and store only the events corresponding to this cuts:
// -- mww < 125 [GeV]/c^2
// -- 120 < mbb < 125 [GeV]/c^2
// -- mjj > 50 [GeV]/c^2
// -- deltaR_ljj < 0.9
// -- deltaR_bblj > 2.85 
//
// c++ -o cutreading cutreading.cpp `root-config --cflags --glibs`
#include "LHEF.h"
#include<iostream>
#include "TH1.h"
#include "TFile.h"
...
int main(int argc, char** argv){
	...
	double j_Et, bb_Et;
	double Ht, Htnu, Ptm;
	double deltar_ljj, deltar_bbljj;
	...
	tree->Branch("j1_phi", &j1_phi);
    tree->Branch("j2_eta", &j2_eta);
    tree->Branch("j2_pt", &j2_pt);
	tree->Branch("j2_phi", &j2_phi);    
...
//                         << endl;
			...
			if(jets.size() > 0 && jets.size() < 3 && tau == 0){      
            	for (int i=0; i<jets.size(); i += 2){
						...
						//Cuts on masses and angles
						if( mww <= 125. && mbb >= 120. && mbb <= 130. && mjj >= 50.){
							if(deltar_ljj < 0.9 && deltar_bbljj > 2.85){								
								tree->Fill();
							}
						}
